...
class VEvent {
  ...
  parseRow (row) {
    ...
    if (keyUpper.startsWith('DTSTART')) {
      this._event.dtstart = parseIcsDate(value)
    } else if (keyUpper.startsWith('DTEND')) {
      this._event.dtend = parseIcsDate(value)
    } else if (VEVENT_PROPS.includes(key)) {
      this._event[key.toLowerCase()] = value
    }
  }
